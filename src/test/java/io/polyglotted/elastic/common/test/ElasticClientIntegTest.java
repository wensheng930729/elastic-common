...
public class ElasticClientIntegTest {
    ...
    public void testHighLevelClient() throws Exception {
        try (ElasticClient elasticClient = highLevelClient(esSettingsBuilder().userName("elastic").password("SteelEye").build())) {
            ...
            checkCreateIndex(elasticClient);
            checkDeleteIndex(elasticClient);
        }
    }
    ...
    private static void checkCreateIndex(ElasticClient client) {
        CreateIndexRequest request = new CreateIndexRequest("customer").source(readResource(ElasticClientIntegTest.class,
            "test-settings.json"),XContentType.JSON);
        CreateIndexResponse response = client.createIndex(request);
        assertTrue(response.isAcknowledged()&& response.isShardsAcknowledged());
        client.dropIndex("customer");
    }

    private static void checkDeleteIndex(ElasticClient client) {
        client.createIndex(new CreateIndexRequest("customer"));
        DeleteIndexResponse response = client.dropIndex("customer");
        assertTrue(response.isAcknowledged());
    }

    @SneakyThrows
    private static String readResource(Class<?> clazz, String resource) { return Resources.toString(urlResource(clazz, resource), UTF_8); }

    private static URL urlResource(Class<?> clazz, String resource) { return clazz.getClassLoader().getResource(resource); }
}